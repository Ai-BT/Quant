...
#### 2-4. 분봉 SMA 골든크로스 전략 ⭐ NEW!
- 1분봉 데이터로 5분/30분 이동평균선 계산
- 1시간 간격으로 거래 신호 확인
- 단기 트레이딩, 당일 매매에 적합
- 거래 간격 조절 가능
...
#### 분봉 SMA 전략 백테스팅 (5분/30분, 1시간 간격 거래)
```bash
python run_sma_minute.py
```

**설정 변경**: `config/sma_minute_config.py` 파일 수정
- `FAST_PERIOD`: 단기 이동평균 (기본 5분)
- `SLOW_PERIOD`: 장기 이동평균 (기본 30분)
- `TRADE_INTERVAL`: 거래 확인 간격 (기본 60분 = 1시간)
- `MARKET`: 코인 변경
